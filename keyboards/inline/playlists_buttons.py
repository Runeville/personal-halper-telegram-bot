...
def get_playlists_navigation(user_id):
...
def get_videos_navigation(playlist_id):
    ...
    prev_video = DBManager().select_video_by_serial_number(playlist_id, int(current_video['serial_number']) - 1)
    next_video = DBManager().select_video_by_serial_number(playlist_id, int(current_video['serial_number']) + 1)
    ...
    markup.add(InlineKeyboardButton(text="Edit name", callback_data=make_callback("edit", playlist_id)))
    markup.insert(InlineKeyboardButton(text="Delete", callback_data=make_callback("delete", playlist_id)))
